...
impl MeterGroup {
    ...
    fn set(&mut self, p: Point, curmax: (isize, isize)) {
        let h = &mut self.0;
    ...
    fn get(&self, p: Point) -> Option<&Meter> {
        let h = &self.0;
        h.get(&p)
    }
}
...
fn look(player: &mut Player, args: &[String]) -> String {
    match args.len() {
...
fn say(player: &mut Player, args: &[String]) -> String {
    format!(r#"you say "{}""#, args.join(" "))
}

fn status(player: &mut Player, args: &[String]) -> String {
...
fn main() -> Result<(), std::io::Error> {
    ...
    loop {
        ...
        println!("> ");
        io::stdin()
            .read_line(&mut tell_me)?;

        let s = tell_me.trim_end();

        println!("{}", c.execute_string(&mut bill, &s.to_string()));
    }
